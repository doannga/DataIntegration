...
import json
import re
class Normalize_careers:
	"""docstring for ClassName"""
	def normalize_careers(self, X):
		career_dict = {
			...
			"cơ khí ô tô tự động hóa": "Cơ khí – Chế tạo – Tự động hoá – Kỹ thuật ứng dụng",
		...
		ca = []
		for x in X:
			x = x.lower().replace("-"," ").replace("/"," ").replace("("," ").replace(")"," ").replace(","," ").strip()
			career = re.sub("\s\s+" , " ", x)
			if career in career_dict:
				ca.append(career_dict[career])
		print(ca)
		career_normailzed = list(dict.fromkeys(ca)) #loai bo cac career trung trong list
		return career_normailzed